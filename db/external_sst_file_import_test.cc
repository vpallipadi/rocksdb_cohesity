...
class ExternalSSTFileTest : public DBTestBase {
 public:
  ...
  Status DeprecatedAddFile(
    ...
    ImportExternalFileOptions opts;
    opts.move_files = move_files;
    return db_->ImportExternalFile(column_family, *import_files_metadata, opts);
  }
...
TEST_F(ExternalSSTFileTest, Basic) {
  do {
    ...
    // sequence number
    {
      std::vector<ImportFileMetaData> import_files_metadata;
      import_files_metadata.push_back(
          ImportFileMetaData(file1, 0, 20, 29));
      import_files_metadata.push_back(
          ImportFileMetaData(file2, 0, 10, 19));
      import_files_metadata.push_back(
          ImportFileMetaData(file3, 0, 0, 0));
    ...
    // sequence number
    {
      std::vector<ImportFileMetaData> import_files_metadata;
      import_files_metadata.push_back(
          ImportFileMetaData(file1, 0, 30, 39));
      import_files_metadata.push_back(
          ImportFileMetaData(file2, 0, 10, 19));
      import_files_metadata.push_back(
          ImportFileMetaData(file3, 0, 20, 29));
    ...
    // Add file1..file3 with file2-file3 overlap and file 3 at higher level
    {
      ...
      import_files_metadata.push_back(
          ImportFileMetaData(file2, 0, 10, 19));
      import_files_metadata.push_back(
          ImportFileMetaData(file3, 1, 20, 29));
    ...
    // Add file1..file3 with file2-file3 overlap and file 2 at higher level
    {
      ...
      import_files_metadata.push_back(
          ImportFileMetaData(file1, 0, 30, 39));
      import_files_metadata.push_back(
          ImportFileMetaData(file2, 2, 10, 19));
      import_files_metadata.push_back(
          ImportFileMetaData(file3, 1, 20, 29));
    ...
    {
      std::vector<ImportFileMetaData> import_files_metadata_0;
      import_files_metadata_0.push_back(
          ImportFileMetaData(file2, 2, 0, 0));
      import_files_metadata_0.push_back(
          ImportFileMetaData(file3, 1, 0, 0));

      s = DeprecatedAddFile(&import_files_metadata_0, false);
      ASSERT_TRUE(s.ok()) << s.ToString();

      std::vector<ImportFileMetaData> import_files_metadata_1;
      import_files_metadata_1.push_back(
          ImportFileMetaData(file1, 0, 0, 99));

      s = DeprecatedAddFile(&import_files_metadata_1, false);
      ASSERT_TRUE(s.ok()) << s.ToString();

      std::vector<ImportFileMetaData> import_files_metadata_2;
      import_files_metadata_2.push_back(
          ImportFileMetaData(file4, 0, 100, 199));
    ...
    // Add file1..5 at different levels
    {
      std::vector<ImportFileMetaData> import_files_metadata;
      import_files_metadata.push_back(
          ImportFileMetaData(file1, 4, 0, 9));
      import_files_metadata.push_back(
          ImportFileMetaData(file2, 3, 10, 19));
      import_files_metadata.push_back(
          ImportFileMetaData(file3, 2, 20, 29));
      import_files_metadata.push_back(
          ImportFileMetaData(file4, 1, 30, 39));
      import_files_metadata.push_back(
          ImportFileMetaData(file5, 0, 40, 49));
    ...
    // Add file1..5 at different levels with overlaps at certain level
    {
      std::vector<ImportFileMetaData> import_files_metadata;
      import_files_metadata.push_back(
          ImportFileMetaData(file1, 4, 0, 0));
      import_files_metadata.push_back(
          ImportFileMetaData(file2, 3, 0, 0));
      import_files_metadata.push_back(
          ImportFileMetaData(file3, 3, 0, 0));
      import_files_metadata.push_back(
          ImportFileMetaData(file4, 1, 0, 0));
      import_files_metadata.push_back(
          ImportFileMetaData(file5, 0, 0, 0));
...
TEST_F(ExternalSSTFileTest, FileWithCFInfo) {
...

  {
    ...
    std::vector<ImportFileMetaData> import_files_metadata;
    import_files_metadata.push_back(ImportFileMetaData(cf1_sst, 0, 10, 19));
    ...
    ASSERT_EQ(Get(1, "K3"), Get(2, "K3"));
    ASSERT_EQ(Get(1, "K4"), Get(2, "K4"));
  }

  {
    // Import sst file corresponding to unknown cf onto default cf and verify
    std::vector<ImportFileMetaData> import_files_metadata;
    import_files_metadata.push_back(ImportFileMetaData(unknown_sst, 0, 10, 19));
...
TEST_F(ExternalSSTFileTest, IngestExportedSSTFromAnotherCF) {
  ...
  ASSERT_OK(checkpoint->ExportColumnFamilyTables(handles_[1],
                                                 &cf_metadata, export_name));

  std::vector<ImportFileMetaData> import_files_metadata;
  for (const auto& level_metadata : cf_metadata.levels) {
    for (const auto& sst_metadata : level_metadata.files) {
      import_files_metadata.push_back(ImportFileMetaData(
          export_name + sst_metadata.name, level_metadata.level,
          sst_metadata.smallest_seqno, sst_metadata.largest_seqno));
    }
  }
  ASSERT_OK(DeprecatedAddFile(handles_[2], &import_files_metadata, false));
  CreateColumnFamilies({"yoyo"}, options);
  
  ASSERT_OK(DeprecatedAddFile(handles_[3], &import_files_metadata, false));
...
TEST_F(ExternalSSTFileTest, IngestExportedSSTFromAnotherDB) {
  ...
  ASSERT_OK(db_copy->ImportExternalFile(cfh, import_files_metadata,
...
TEST_F(ExternalSSTFileTest, AddListAtomicity) {
  do {
    ...
    std::vector<ImportFileMetaData> import_files_metadata;
    for (int i = 0; i < n; i++) {
      std::string fname = sst_files_dir_ + "file" + std::to_string(i) + ".sst";
      ExternalSstFileInfo files_info;
      ASSERT_OK(sst_file_writer.Open(fname));
      for (int k = i * 100; k < (i + 1) * 100; k++) {
        ASSERT_OK(sst_file_writer.Put(Key(k), Key(k) + "_val"));
      }
      Status s = sst_file_writer.Finish(&files_info);
      ASSERT_TRUE(s.ok()) << s.ToString();
      ASSERT_EQ(files_info.file_path, fname);
      ...
      import_files_metadata.push_back(
          ImportFileMetaData(fname, 0, 0, 0));
    }
    import_files_metadata.push_back(ImportFileMetaData(
        sst_files_dir_ + "file" + std::to_string(n) + ".sst", 0, 0, 0));
...
TEST_F(ExternalSSTFileTest, MultiThreaded) {
  ...
  do {
    ...
    std::function<void()> load_file_func = [&]() {
      ...
      std::vector<ImportFileMetaData> import_files_metadata;
      import_files_metadata.push_back(
          ImportFileMetaData(file_names[file_idx], 1, 0, 0));

      if (static_cast<size_t>(file_idx + 1) < file_names.size()) {
        import_files_metadata.push_back(
            ImportFileMetaData(file_names[file_idx + 1], 1, 0, 0));
      }
...
TEST_F(ExternalSSTFileTest, CompactDuringAddFileRandom) {
  ...
  std::function<void()> bg_addfile = [&]() {
    SstFileWriter sst_file_writer(EnvOptions(), options);
    ASSERT_EQ(sst_file_writer.FileSize(), 0);
    std::string file1 = sst_files_dir_ + "file.sst";
    ASSERT_OK(sst_file_writer.Open(file1));
    ...
    ASSERT_TRUE(s.ok()) << s.ToString();
    std::vector<ImportFileMetaData> import_files_metadata;
    import_files_metadata.push_back(ImportFileMetaData(file1, 0, 0, 0));
    ASSERT_OK(DeprecatedAddFile(&import_files_metadata, false));
  };
...
TEST_F(ExternalSSTFileTest, AddExternalSstFileWithCustomCompartor) {
  ...
  // {44 -> 30}
  // ..
  std::vector<ImportFileMetaData> generated_files_metadata;
  for (int i = 0; i < 10; i++) {
    ...
    generated_files_metadata.push_back(ImportFileMetaData(file_name, 1, 0, 0));
  }
